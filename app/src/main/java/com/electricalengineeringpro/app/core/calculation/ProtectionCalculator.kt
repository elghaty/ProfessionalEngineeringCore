...
class ProtectionCalculator {
    ...
    fun calculate(input: ProtectionInput): ProtectionResult {

        require(input.designCurrentA > 0)
        require(input.cableAmpacityA > 0)
        require(input.shortCircuitKA >= 0)
    ...
    private fun standardBreaker(current: Double): Double {
        ...
        return ratings.firstOrNull { it >= current }
            ?: ceil(current / 100.0) * 100.0
    }

    private fun standardBreakingCapacity(shortCircuitKA: Double): Double {
        val capacities = listOf(
            3.0, 6.0, 10.0, 15.0, 18.0, 25.0,
            30.0, 36.0, 40.0, 50.0, 65.0, 80.0,
            100.0
        )

        return capacities.firstOrNull { it >= shortCircuitKA }
            ?: 100.0
    }
}
